...
export default function AboutSection() {
  ...
  const [activeSkillName, setActiveSkillName] = useState<string>("SYSTEM_ROOT");
  ...
  const handleHover = (skill: Skill | null) => {
    if (skill) {
      ...
      setActiveSkillName("SYSTEM_ROOT");
    }
  };
    ...
    >
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16">
        ...
        <div className="lg:col-span-7 flex flex-col gap-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {category.skills.map((skill) => (
                    <button
                      ...
                      className="font-mono text-xs px-3 py-2 border border-surface bg-surface/10 rounded transition-all duration-150 hover:bg-accent/10 hover:border-accent hover:text-primary text-muted focus:outline-none focus:border-accent"
                    >
                      {skill.name}
                    </button>
                  ))}
          ...
          <div className="w-full bg-surface border border-surface rounded p-4 font-mono text-xs shadow-inner">
            <div className="flex items-center justify-between border-b border-surface pb-2 mb-3 text-[var(--color-text-muted)]">
